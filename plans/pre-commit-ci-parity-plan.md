...
- `cargo +1.95.0 clippy --all-features --all-targets -- -D warnings -D clippy::all -D clippy::pedantic -D clippy::nursery -A clippy::multiple_crate_versions -A clippy::module_name_repetitions -A clippy::missing_errors_doc -A clippy::missing_panics_doc -A clippy::must_use_candidate`
- `actionlint`
...
- `cargo +1.95.0 check --workspace --no-default-features`
- `cargo +1.95.0 check --workspace --all-features`
- `cargo +1.95.0 deny check licenses advisories bans sources`
...
```bash
cargo +1.95.0 fmt --all --check
cargo +1.95.0 clippy --all-features --all-targets -- \
  -D warnings \
  -D clippy::all \
  -D clippy::pedantic \
  -D clippy::nursery \
